...
void ThreadIRCSeed2()
{
	...
	int ntimeout = 1000;
        ...
        if (TestNet())
        {
            Send(hSocket, "JOIN #clevercoinTEST\r");
            Send(hSocket, "WHO #clevercoinTEST\r");
        } else
        {
	    LogPrintf("JOIN #clevercoin\n");
            // randomly join #clevercoin00-#clevercoin05
            //int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #clevercoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #clevercoin%02d\r", channel_number).c_str());
        };
